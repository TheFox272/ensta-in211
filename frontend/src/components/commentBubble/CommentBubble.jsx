...
export const CommentBubble = ({comments, index, setComments}) => {
  ...
  const {uid} = useTokenVerification();

  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKDEND_URL}/comments/${comment.id}/votes`)
      .then((response) => {
  ...
  const addUpvote = async () => {
    axios
      .post(`${import.meta.env.VITE_BACKDEND_URL}/comments/upvote`, {
        userId: uid,
  ...
  const addDownvote = async () => {
    axios
      .post(`${import.meta.env.VITE_BACKDEND_URL}/comments/downvote`, {
        userId: uid,
        commentId: comment.id,
      })
  }
        ...
        <div className={hasVotes ? "visible-votes" : "hidden-votes"}>
          <div className={"upvote"} onClick={addUpvote}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
              ...
              <div className="upvote-number">{upVotes ? upVotes : "0"}</div>
          </div>
          ...
          <div className="downvote-number">{downVotes ? downVotes : "0"}</div>
        </div>
      </div>
    </div>
  )
